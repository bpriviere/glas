function [verts] = swept_cyl_verts(cyl,waypoints)
    ...
    nsteps = size(waypoints,1) - 1;
    verts = zeros(nsteps*16,3);

    %generate verts for each timestep
    for t = 1:nsteps
        %Translation direction
        dir = waypoints(t+1,:) - waypoints(t,:);

        %dirxy is used to orient polytope hull vertices in direction of travel
        dirxy = waypoints(t+1,1:2) - waypoints(t,1:2);
        ...
        %bottom vertices
        bottom = [[v1dir*radius*sqrt(2),0] + waypoints(t,:) - [0,0,below]; ...
                     [v2dir*radius*sqrt(2),0] + waypoints(t,:) - [0,0,below]; ...
                     [v3dir*radius*sqrt(2),0] + waypoints(t,:) - [0,0,below]; ...
                     [v4dir*radius*sqrt(2),0] + waypoints(t,:) - [0,0,below]];

        verts0 = [bottom; bottom + repmat([0,0,height],[4,1])]; %verts from t
        verts1 = verts0 + dir; %verts from t+1
        
        verts(((t-1)*16 + 1):(t*16),:) = [verts0;verts1];
        %hullVerts = allVerts;
